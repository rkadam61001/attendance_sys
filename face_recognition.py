# import re
from sys import path
from tkinter import*
from tkinter import ttk
from PIL import Image,ImageTk
import os
import mysql.connector
import cv2
import numpy as np
from tkinter import messagebox
from time import strftime
from datetime import datetime
import smtplib
import winsound

from databaseTest import create_connection

class Face_Recognition:
    # sender_email = "rajnandini.kadam@walchandsangli.ac.in"
    # print(sender_email)
    # password = "Pratibha@123"
    # server = smtplib.SMTP('smtp.gmail.com', 587)
    # server.starttls()
    # server.login(sender_email, password)
    # print("Login successful to your gmail account")
    is_mobile = '0'
    
    def __init__(self,root):
        self.root=root
        self.root.geometry("1366x768+0+0")
        self.root.title("Face Recognition Pannel")

        # set image as lable
        f_lb1 = Label(self.root,bg="black")
        f_lb1.place(x=0,y=0,width=1366,height=130)


        # set image as lable
        bg_img = Label(self.root,bg="mint cream")
        bg_img.place(x=0,y=130,width=1366,height=768)


        #title section
        title_lb1 = Label(bg_img,text="Welcome to Face Recognition Pannel",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        title_lb1.place(x=0,y=0,width=1366,height=45)

        # Create buttons below the section 
        # ------------------------------------------------------------------------------------------------------------------- 
        # Training button 1
        std_img_btn=Image.open(r"Images_GUI\f_det.jpg")
        std_img_btn=std_img_btn.resize((180,180),Image.Resampling.LANCZOS)
        self.std_img1=ImageTk.PhotoImage(std_img_btn)

        std_b1 = Button(bg_img,command=self.face_recog,image=self.std_img1,cursor="hand2")
        std_b1.place(x=600,y=170,width=180,height=180)

        std_b1_1 = Button(bg_img,command=self.face_recog,text="Face Detector",cursor="hand2",font=("tahoma",15,"bold"),bg="white",fg="navyblue")
        std_b1_1.place(x=600,y=350,width=180,height=45)

        std_b1_1 = Button(bg_img,command=self.face_recog_by_mobile,text="By mobile device",cursor="hand2",font=("tahoma",15,"bold"),bg="white",fg="navyblue")
        std_b1_1.place(x=600,y=450,width=180,height=45)


        exi_img_btn = Image.open(
            r"Images_GUI\quit.jpeg")
        exi_img_btn = exi_img_btn.resize((50, 50), Image.Resampling.LANCZOS)
        self.exi_img1 = ImageTk.PhotoImage(exi_img_btn)

        exi_b1 = Button(bg_img, command=self.Close,
                        image=self.exi_img1, cursor="hand2",)
        exi_b1.place(x=1290, y=580,height=50,width=50)

    #=====================Attendance===================
    def sound():
        print("reasched")
        frequency = 2500
        duration = 1000
        winsound.Beep(frequency,duration)

    

    def mark_attendance(self,i,r,n):
        
        now = datetime.now()
        currentDate = now.strftime("%Y-%m-%d")

        with open( "attendance.csv","r+",newline="\n") as f:
            myDatalist=f.readlines()
            name_list=[]
            for line in myDatalist:
                entry=line.split((","))
                name_list.append(entry[0])
                
                

            if((i not in name_list)) and ((r not in name_list)) and ((n not in name_list)):
                now=datetime.now()
                d1=now.strftime("%d/%m/%Y")
                dtString=now.strftime("%H:%M:%S")
                f.writelines(f"\n{i}, {r}, {n}, {dtString}, {d1}, Present")
                conn = create_connection()
                cursor = conn.cursor()
                try:
                    cursor.execute("select Email from face_recognition.student where Student_ID="+i)
                
                    student_mail=cursor.fetchone()
                    if(student_mail!=None):

                        Face_Recognition.server.sendmail(Face_Recognition.sender_email, student_mail , "Your Attendance has been Marked")
                        print("Email sent to ", student_mail)
                        Face_Recognition.sound()
                except Exception as e:
                    print(e)

    def face_recog_by_mobile(self):
        self.is_mobile='1'
        self.face_recog()
        return "0"

    #================face recognition==================
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            
            recognizer = cv2.face.LBPHFaceRecognizer_create()
            recognizer.read("clf.yml")

            if img is not None:
                gray_image=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
                featuers=classifier.detectMultiScale(gray_image,scaleFactor,minNeighbors)

                coord=[]
                
                for (x,y,w,h) in featuers:
                    cv2.rectangle(img,(x,y),(x+w,y+h),(0,255,0),2)
                    # id, confidence = recognizer.predict(gray_image[y:y+h,x:x+w])
                    id,predict=clf.predict(gray_image[y:y+h,x:x+w])

                    confidence=int((100*(1-predict/300)))

                    conn = create_connection()
                    cursor = conn.cursor()
                    try:
                        cursor.execute("select Name from student where Student_ID=" + str(id))
                        n = cursor.fetchone()
                        if n is not None:
                            n = "+".join(list(n))
                            # print("Student name: ", n)
                        # else:
                        #     print("No student found with Name:", id)
                    except Exception as e:
                        print("Error:", e)

                    try:
                        cursor.execute("select Roll_No from student where Student_ID=" + str(id))
                        r = cursor.fetchone()
                        if r is not None:
                            r = "+".join(list(r))
                            # print("Student Roll_No: ", r)
                        # else:
                        #     print("No student found with ID:", id)
                    except Exception as e:
                        print("Error:", e)


                    try:
                        cursor.execute("select Student_ID from student where Student_ID=" + str(id))
                        i = cursor.fetchone()
                        if i is not None:
                            i = "+".join(list(i))
                            # print("Student Student_ID: ", i)
                        # else:
                        #     print("No student found with ID:", id)
                    except Exception as e:
                        print("Error:", e)

                    # print(confidence)
                    if(n!=None):
                        if confidence > 77:
                            cv2.putText(img,f"Student_ID:{i}",(x,y-80),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
                            cv2.putText(img,f"Name:{n}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
                            cv2.putText(img,f"Roll-No:{r}",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
                            if(i!=None):
                                self.mark_attendance(i,r,n)
                        else:
                            cv2.rectangle(img,(x,y),(x+w,y+h),(0,0,255),3)
                            cv2.putText(img,"Unknown Face",(x,y-5),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,0),3)    

                    coord=[x,y,w,y]
                
            return coord    


        #==========
        def recognize(img,clf,faceCascade):
            coord=draw_boundray(img,faceCascade,1.1,10,(255,25,255),"Face",clf)
            return img
        
        faceCascade=cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
        clf=cv2.face.LBPHFaceRecognizer_create()
        clf.read("clf.yml")

        if(self.is_mobile=='0'):
            videoCap=cv2.VideoCapture(0)
        else:
            videoCap=cv2.VideoCapture('https://192.168.23.125:8080/video')

        while True:
            ret,img=videoCap.read()
            img = cv2.resize(img,(640,480))
            img=recognize(img,clf,faceCascade)
            cv2.imshow("Face Detector",img)

            if cv2.waitKey(1) == 13:
                break
        videoCap.release()
        cv2.destroyAllWindows()


    def Close(self):
            self.root.destroy()


if __name__ == "__main__":
    root=Tk()
    obj=Face_Recognition(root)
    root.mainloop()